(function() {
  ...
  angular
      .module('cpccore')
      .controller('TimelineTeamCtrl', TimelineTeamCtrl);

  TimelineTeamCtrl.$inject = ['dataservice', 'localStorageService', '$http', '$rootScope','objectStore', '$state', '$stateParams'];

  /* @ngInject */
  function TimelineTeamCtrl(dataservice, localStorageService, $http, $rootScope, objectStore, $state, $stateParams) {

    var vm = this;
    vm.loading = true;
    vm.showLoadMore = false;
    vm.timelineTeamSecondNavigate = timelineTeamSecondNavigate;
    vm.isAlternateManager = localStorageService.get("loggedInUser").isAlternateMgr;
    ...
    function timelineTeamSecondNavigate(){
      $state.go('timeline.teamSecondState');
    }
    ...
    function activate(){
      ...
      vm.loading = true;
      dataservice.getTimeline(vm.currentPage, vm.recordsPerPage, loginuser, vm.c_yr, vm.c_q, "TEAM", vm.sort).then(function (response) {
