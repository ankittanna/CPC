(function() {
  ...
  angular
    .module('cpccore')
    .controller('AlternateManagerTeamCtrl', AlternateManagerTeamCtrl);

  /* @ngInject */
  function AlternateManagerTeamCtrl(localStorageService, dataservice, $http, $filter, employeeDetails, objectStore, $state, growl, $stateParams) {
    ...
    function alternateTeamSecondNavigate(){
      if($stateParams.username){
        $state.go('teamMemberAlternateManager.teamSecondState',{"tmusername":$stateParams.username});
      }
      else{
        $state.go('alternateManager.teamSecondState');
      }
    }
