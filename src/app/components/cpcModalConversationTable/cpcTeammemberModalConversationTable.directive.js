(function() {
  ...
  angular
    .module('cpccore')
    .directive('cpcTeammemberModalConversationTable', cpcTeammemberModalConversationTable);

  /* @ngInject */
  function cpcTeammemberModalConversationTable() {
    // Usage:
    //
    // Creates:
    //
  ...
  /* @ngInject */
  function Controller(dataservice, _, $http, $location,$state,$stateParams,localStorageService,objectStore) {
    var vm = this;
    ...
    if($stateParams.username){
    	vm.selectedUser = $stateParams.username;
    }
    activate();
    vm.go = function ( path ) {
	  	$location.path( path );
...
//    				for(var q in quartersInFy){
    ...
    function activate() {
      ...
      dataservice.getConversationTeam("TEAM", vm.currentPage, vm.recordsPerPage,vm.selectedUser,c_yr, c_q).then(function (response) {
                vm.loading = false;
