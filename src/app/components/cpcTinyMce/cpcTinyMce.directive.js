(function() {
  ...
  function cpcTeam() {
    ...
    //
    var directive = {
      templateUrl: '/client/app/components/cpcTinyMce/cpcTinyMce.html',
  ...
  function Controller($log, $translate, $scope, $modal, $window,objectStore) {
	//  	$scope.currentConversation = $window.draftConversationObject;
    var vm = this;

    vm.max_chars    = 4000; //max characters
    vm.max_for_html = 5000; //max characters for html tags
    ...
    vm.alarmChars = function(){
     ...
     vm.tinyMceIntitialized = function()
    {
      console.log("TINY MCE INITIALIZED");

       vm.draftConversationObject = objectStore.conversation.draftConversation.get();
        console.log("TinyMCE Draft");
        console.log(objectStore.conversation.draftConversation.get());
      if(typeof vm.draftConversationObject !== undefined && vm.draftConversationObject !== null){
        console.log("****** in tiny mce "+vm.draftConversationObject);
        console.log("comment section "+vm.draftConversationObject.conversation[0].comments);
        $scope.currentConversation = vm.draftConversationObject;

        if($scope.currentConversation.conversation.length === 1)
        {
          if($scope.currentConversation.conversation[0].status === 'DRAFT')
          {
            if($scope.currentConversation.conversation[0].comments != '')
            {
              tinyMCE.activeEditor.setContent($scope.currentConversation.conversation[0].comments);
            }
          }
        } else if($scope.currentConversation.conversation.length > 1)
        {
          if($scope.currentConversation.conversation[1].status === 'DRAFT')
          {
            if($scope.currentConversation.conversation[1].comments != '')
            {
              tinyMCE.activeEditor.setContent($scope.currentConversation.conversation[1].comments);
            }
          }
        }
        }
      ...
      tinymce.init({
		  ...
		  init_instance_callback: vm.tinyMceIntitialized,
          style_formats: [{
              title: 'Bold text',
              inline: 'b'
          }, {
          ...
          setup: function(ed) {
              ed.on('init', function(args) {
                  ed.setContent("Enter Sync Up Details");
              });

              ed.on('focus', function(args) {
                  if (ed.getContent().length != 0 && ed.getContent() == "<p>Enter Sync Up Details</p>") {
                      ed.setContent('');
                  }
              });
      ...
      // Preload the Tiny MCE with draft content
